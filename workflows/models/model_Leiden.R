...
getLeidenClusters <- function(scData, numClusters = 6, resolution_start=0.1){
    currNumClusters <- 0
    ...
    while (currNumClusters < numClusters) {
        clusters <- Seurat::FindClusters(scData, resolution = resolution, algorithm=4)$seurat_clusters
...
clusters <- getLeidenClusters(scData, numClusters = length(unique(scData$ground_truth)))
